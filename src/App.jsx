...
5. PADRÃO DE DESCRIÇÃO E COMPETÊNCIA DAS DESPESAS FIXAS (OBRIGATÓRIO quando houver Razão com descrições — NÃO pule esta verificação): a descrição de cada lançamento de despesa fixa (serviços terceirizados, energia, água, gás, salários, encargos, contabilidade, elevadores, portaria, limpeza, seguro, internet) DEVE seguir o padrão "[serviço] - [mês/ano de competência] - NF:[número]". Exemplo correto: "Serviços terceirizados mensal - Jul/26 - NF:1234". Percorra TODAS as despesas fixas do Razão e preencha "descricoes_fora_padrao" com CADA lançamento cuja descrição NÃO siga o padrão — em especial os que estão SEM o mês/ano de competência ou SEM o número da NF. Para cada um, informe a conta, a descrição atual e o que está faltando. Apenas aponte, não sugira a descrição corrigida. Se a maioria seguir o padrão e só algumas fugirem, liste só as que fugiram. Só deixe a lista vazia se realmente TODAS estiverem no padrão OU se não houver Razão com descrições.`;
      const resp = await chamarClaude(prompt, arquivos);
      const json = parseJSON(resp);
